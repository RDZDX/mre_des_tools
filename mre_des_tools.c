...
VMINT job(VMWCHAR *filepath, VMINT wlen) { //encrypt
    ...
    vm_ascii_to_ucs2(ext, sizeof(ext), "_txt.des");
    vm_wstrncpy(outfile, filepath, vm_wstrlen(filepath) - 4);
...
VMINT job1(VMWCHAR *filepath, VMINT wlen) { //decrypt
    ...
    vm_ascii_to_ucs2(ext, sizeof(ext), "_des.txt");
    vm_wstrncpy(outfile, filepath, vm_wstrlen(filepath) - 4);
    vm_wstrcat(outfile, ext);
