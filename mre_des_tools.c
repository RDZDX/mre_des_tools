...
void save_text(VMINT state, VMWSTR text) {
    ...
    if (state == VM_INPUT_OK && lenght > 0) {
        ...
        check_selector = vm_selector_run(0, 0, job);  // BIN to DES
        //check_selector = vm_selector_run(0, 0, job1); // DES to BIN
...
VMINT job(VMWCHAR *filepath, VMINT wlen) {
    ...
    VMINT i;
    VMWCHAR filename_ucs2[260];
    VMWCHAR output_wpath[260];
    char filename_ascii[260];
    char output_ascii[260];

    // Gauti originalų failo pavadinimą
    vm_get_filename(filepath, filename_ucs2);
    vm_ucs2_to_ascii(filename_ascii, sizeof(filename_ascii), filename_ucs2);

    // Sukurti output ASCII kelią
    vm_ucs2_to_ascii(output_ascii, sizeof(output_ascii), filepath);
    strcat(output_ascii, ".des");

    // Konvertuoti į UCS2
    vm_ascii_to_ucs2(output_wpath, sizeof(output_wpath), output_ascii);
    ...
    f_write = vm_file_open(output_wpath, MODE_CREATE_ALWAYS_WRITE, TRUE);
